...
1. 读清单并检查实际作者覆盖。固定同一返回版本完成本次分析；过程中版本改变则重查所用知识。
...
5. 相关性门槛仍可能有误配，Agent 须检查是否真正回答问题。无关结果应舍弃；不足可改问法一次，仍不足就明确未覆盖。
...
先回答问题，再解释有帮助的书中视角，给少量可执行步骤。书中主张就近注明书名、章节、知识或证据 ID；把迁移步骤写成“据此可尝试”等系统建议。引用不足、条件差异和模型缺乏信息须在相应结论旁说明。不擅自创建提醒或触发外部操作。
